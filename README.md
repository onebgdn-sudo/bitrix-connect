...
Для постоянной установки используй один хост для фронта и backend вместе:

```bash
docker build -t carte-showbiz .
docker run --rm -p 8787:8787 --env-file .env carte-showbiz
```

Потом подставь этот постоянный URL в Bitrix как handler URL для `bitrix-entry.html`.
Не используй временные туннели для постоянной работы.
...
Для стабильной установки нужен постоянный домен. Временные туннели подходят только для проверки. Front-end получает конфиг через `/runtime-config.json`, а backend отвечает на `/healthz`, чтобы было проще отслеживать живую установку и переносить приложение между хостами без переписывания логики.
